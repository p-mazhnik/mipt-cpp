...
public:
    ...
    Treap *left;
    Treap *right;
    Treap(int key) : key(key), priority(rand()), size(1), sum(key), left(NULL), right(NULL){};
    Treap() : left(NULL), right(NULL){};
};

void swap(Treap *odd, Treap *even, int l, int r);
...
long long sum(Treap* odd, Treap* even, int l, int r) {
    if (l == 0 && r == 0){
        return sum(odd, ((l) / 2), r / 2);
    }
    else {
        return sum(even, (l + 1) / 2, (r) / 2) + sum(odd, (l) / 2, r / 2);
    }
}
...
void my_swap(Treap* &odd, Treap* &even, int l, int r) {
    ...
    split(odd, r / 2, one_odd, two_odd);
    split(one_odd, (l) / 2 - 1, three_odd, four_odd);

    Treap *one_even, *two_even;
    Treap *three_even, *four_even;
    split(even, (r) / 2, one_even, two_even);
    split(one_even, (l + 1) / 2 - 1, three_even, four_even);
...
int main(){
    ...
    while(n != 0 && m != 0) {
        ...
        for (int i = 0; i < n; i++) { //считываем последовательность
            std::cin >> value;
            if (i % 2 == 0){
                insert(even, value, (i) / 2 + 1);
            }
            else {
                insert(odd, value, (i) / 2 + 1);
            }
        }
